...
// Function to calculate the Longest Proper Prefix which is also a Suffix (LPS) for pattern 'p'
void lps(string p, vector<int>& temp)
{
    ...
    while (j < temp.size())
    {
        ...
        {
            i++;
            temp[j] = i; // LPS for the current character is the length of the previous longest prefix suffix + 1
            j++;
        }
        else
        {
            if (i != 0)
                i = temp[i - 1]; // If there is a mismatch, move 'i' to the previous longest prefix suffix and continue comparing
            else
            {
                temp[j] = 0; // If 'i' is already at 0, set LPS for the current character as 0 and move to the next character
                j++;
            }
        }
    }
...
bool findPattern(string p, string s)
{
    ...
    vector<int> temp(n); // Create an array to store the LPS values for the pattern
    ...
    while (j < m)
    {
        ...
        if (i == n) // If all characters of the pattern have been matched, pattern found
            return true;
        else if (j < m && s[j] != p[i]) // If characters don't match
        {
            if (i != 0)
                i = temp[i - 1]; // Move 'i' to the previous longest prefix suffix and continue comparing
            else
